...
class ChatState extends AppState {
  ...
  MyUser get chatUser => _chatUser;
  set setChatUser(MyUser model) {
    _chatUser = model;
  }
  ...
  void onMessageSubmitted(ChatMessage message,
      {MyUser myUser, MyUser secondUser}) {
  ...
  void dispose() {
    if (_chatUserList != null) {
//      print(_chatUserList.length);
      var u = _chatUserList.where((x) {
        if (x.key == chatUser.userId)
          return true;
        else
          return false;
      });
...
//      print(u);
      if (u.isNotEmpty)
        ...
        user = null;
      if (user != null && _messageList != null) {
